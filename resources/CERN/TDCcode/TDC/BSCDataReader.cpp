...
void BSCDataReader::Run(){
  ...
  while( !fStopFlag ){
    for( unsigned mod = 0; mod < _modules.size(); ++mod ) fStopFlag = fStopFlag || !_modules[ mod ]->Read();
        ...
        // Writing the event number
        if( SpillCount % 50 == 0 ) {

           if( (stream = fopen( "/home/user/temp/tdc_eventnumber", "w" )) != NULL ) {
              fprintf (stream, "%d\n",SpillCount);
              fflush( stream );
              fclose( stream );
           }
           ...
           // Stopping nicely the run
           stream = fopen("/home/user/temp/tdc_stop","r");
           fscanf (stream, "%d\n",&tdcstatusonoff);
           fflush( stream );
           fclose(stream);
           if ( tdcstatusonoff == 1 ) fStopFlag = true;
       }
  }

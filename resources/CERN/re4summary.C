...
void re4summary() {
    ...
    int volt_bin = 100, volt_min = 8000, volt_max = 10200;
    ...
    Float_t     efficiency, chambercsize[200], vth[200] ,efficiencyerr, csize, csize_out, vth_out, noise, Volt, hvtn, hvtw, hvb, var1, imontn, imontw, imonb, noise;
    
    Float_t vth1_a, vth2_a, vth3_a, vth4_a, vth1_b, vth2_b, vth3_b, vth4_b, vth1_c, vth2_c, vth3_c, vth4_c;
    
    Int_t       run, numberofevents;
    Char_t      trigger_mode[20], eta[20], name[200], site[80], type[80], trigger_mode_out[80], eta_out[80], name_out[200], site_out[80], type_out[80];
    
    
    // Output file variables
    Float_t fit_efficiency, fit_efficiency_err, slope, slope50, slope_err, hvat50, hvat50_err, chi2, hv50_diff_ab, hv50_diff_ac, hv50_diff_bc, hv50_diff_tb, hv95_diff_ab, hv95_diff_ac, hv95_diff_bc, hv95_diff_tb, hv9010_diff_ab, hv9010_diff_ac, hv9010_diff_bc, hv9010_diff_tb, hv7525_diff_ab, hv7525_diff_ac, hv7525_diff_bc, hv7525_diff_tb, effi_diff_ab, effi_diff_ac, effi_diff_bc, effi_diff_tb;
    ...
    rpclab->Branch("imontw",&imontw,"imontw/F");
    rpclab->Branch("imontn",&imontn,"imontn/F");
    rpclab->Branch("imonb",&imonb,"imonb/F");
    
    rpclab->Branch("vth1_a",&vth1_a,"vth1_a/F");
    rpclab->Branch("vth2_a",&vth2_a,"vth2_a/F");
    rpclab->Branch("vth3_a",&vth3_a,"vth3_a/F");
    rpclab->Branch("vth4_a",&vth4_a,"vth4_a/F");
    rpclab->Branch("vth1_b",&vth1_a,"vth1_b/F");
    rpclab->Branch("vth2_b",&vth2_a,"vth2_b/F");
    rpclab->Branch("vth3_b",&vth3_a,"vth3_b/F");
    rpclab->Branch("vth4_b",&vth4_a,"vth4_b/F");
    rpclab->Branch("vth1_c",&vth1_a,"vth1_c/F");
    rpclab->Branch("vth2_c",&vth2_a,"vth2_c/F");
    rpclab->Branch("vth3_c",&vth3_a,"vth3_c/F");
    rpclab->Branch("vth4_c",&vth4_a,"vth4_c/F");
    ...
    rpclab_performance->Branch("vth_out",&vth_out,"vth_out/F");
    ...
    // Verboses printouts
    int verbose_filling_chamber = 0, verbose_loop_chamber = 0;
    ...
    if (myfile.is_open()) {
        while ( myfile.good() ) {
            ...
            // Reading input file
            myfile >> site >> date >> time >> var1 >> run >> name >> type >> eta >> trigger_mode >> numberofevents >> Volt >> hvtn >> hvtw >> hvb >> imontn >> imontw >> imonb >> efficiency >> efficiencyerr >> noise >> vth1_a >> vth2_a >> vth3_a >> vth4_a >> vth1_b >> vth2_b >> vth3_b >> vth4_b >> vth1_c >> vth2_c >> vth3_c >> vth4_c >> csize;
            //cout << "reading file... " << name << endl;
            
            
            // Solving DB ghent bug
            if (strcmp(name,"CMS-RE4-3-Ghent-003")==0) {
                efficiency = efficiency*100;
                efficiencyerr = efficiencyerr*100;
            }
            
            if (strcmp(name,"CMS-RE4-3-Ghent-006")==0) {
                efficiency = efficiency*100;
                efficiencyerr = efficiencyerr*100;
            }
            ...
            // Filling chamber, site, type vector names
            {
                ...
                if ( j == 1 ) {
                    ...
                    if (Volt==10000) {
                        ...
                        chambercsize[j] = csize;
                    }
                    
                    vth[j] = (vth1_a + vth1_b + vth1_c + vth2_a + vth2_b + vth2_c + vth3_a + vth3_b + vth3_c + vth4_a + vth4_b + vth4_c)/12 ;
                    ...
                    if (verbose_filling_chamber) cout << " I have found the chamber " << chambervector[j] << " " << j << " site = " << sitevector[j] << " type = " << type << "(reading: " << name << endl;
                    j++;
                }
                ...
                if ( j > 1) {
                    ...
                    if ( chambervector[j-1] != chambername ) {
                        chambervector[j] = chambername;
                        if (verbose_filling_chamber) cout << " I have found the chamber " << chambervector[j] << " " << j << " site = " << sitevector[j] << " type = " << typevector[j] << "(reading: " << name << endl;
                        j++;
                    }
        ...
        for (l = 1; l <= trigger_modes; l++ ) {
            ...
            for (k = 1; k <= etas; k++ ) {
                ...
                // Printouts verbose
                {
                    ...
                    cout << " chamber " << chambervector[i] << " (site = " << sitevector[i] << " and type = " << typevector[i] << " )" << " IMON_B " << current1 << endl;

                    
                    //cin >> buttonpress;
                }
                csize_out = chambercsize[i];
                vth_out = vth[i];
                ...
                {
                    strcpy(outputtempfile,"/Users/tetto/Desktop/temp/");
                ...
                {
                    TH2F *hvpoints = new TH2F("hvpoints","hvpoints",1,0,11000,1,0,100);
                ...
                // Perform the fits
                {
                    ...
                    /*
                     ...
                     Double_t arglist[number_of_points];
                     Int_t ierflg = 0;
                    ...
                    slope50 = fit_efficiency * slope*1000 / 4;
                ...
                // Saving the canvas into images
                {
                    ...
                    if ( l == 1 ) etamode_out = "_double";
                    if ( l == 2 ) etamode_out = "_top";
                    if ( l == 3 ) etamode_out = "_bottom";
                    
                    if ( k == 1 ) trgmode_out = "_a";
                    if ( k == 2 ) trgmode_out = "_b";
                    if ( k == 3 ) trgmode_out = "_c";
                    ...
                    //sigmoid_toshow->Draw();
                    Frameperf->Update();
                    Frameperf->SaveAs( "/Users/tetto/Desktop/temp/" + output + etamode_out + trgmode_out + ".gif" );
                }
    ...
    cout << " AAA Chamber " << name_out << " " << eta_out << " fit_effi " << fit_efficiency << endl;
                // Deleting objects
                //Frameperf->Delete();
                //sens1->Delete();
